 ...
 *
 *   node src/provider.js [--mode good|burn|overcharge] [--price 0.01]
 */
...
async function main() {
  const { agent, address } = await createAgent("PROVIDER", { label: "PROVIDER" });
    ...
    try {
      ...
      if (entry.kind === "system") {
        ...
        if (entry.event.type === "job.funded") {
          ...
          log("PROVIDER", `job ${session.jobId} submitted ${a.mode === "burn" ? "a spec-missing" : "a spec-meeting"} deliverable (${text.length} chars)`);
        }
      }
    } catch (err) {
      log("PROVIDER", `error on job ${session.jobId}: ${err.shortMessage || err.message}`);
    }
  });

  await agent.start(() => log("PROVIDER", "connected to ACP event stream"));
}
